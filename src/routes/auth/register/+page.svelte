...
<div class="bg-[#363636] text-white sm:rounded-md p-8 sm:w-[500px] w-full drop-shadow-md h-full sm:h-auto sm:mt-0 z-10 select-none">
    <h2 class="mt-2 sm:mt-0 text-xl sm:text-2xl font-semibold text-center mb-5">Create an Account</h2>
    <form on:submit|preventDefault class="flex flex-col">
        <label for="email" class="text-[#C3C3C3] font-semibold mb-2 text-sm">Email</label>
        <input placeholder="" type="email" bind:value={email} class="bg-[#232323] p-2 rounded-[4px] mb-4"/>
        ...
        <Button label="Continue" onClick={signUp} loading = {loading}/>
    </form>
    <p class="text-sm text-[#B1B1B1] text-center">Have an account? <a class="text-[#65C0BD] font-semibold underline" href="/auth/login">Sign in Now</a></p>
</div>
